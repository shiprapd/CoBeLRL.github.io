...
class FrontendBlenderInterface():
    ...
    def __init__(self, scenarioName):
        """
        Inits Blender interface class

        :param scenarioName: the FULL path to the scenario that should be processed
        :return: None
        """
    ...
    def getManuallyDefinedTopologyNodes(self):
        ...
        :param: None
        :return manuallyDefinedTopologyNodes: topology nodes to avoid
        """
    ...
    def getManuallyDefinedTopologyEdges(self):
        ...
        :param: None
        :return manuallyDefinedTopologyEdges: topology edges to avoid
        """
        # get the forbidden zones' layouts for the robot to avoid
    ...
    def recvImage(self, s, imgSize):
        ...
        :param s:        socket to read image from
        :param imgSize:  the size of the image to read
        :return imgData: image data
        """
        # define buffer
        imgData = b''
        # define byte 'counter'
    ...
    def stepSimulation(self, velocityLinear, omega):
        ...
        :param velocityLinear: the requested linear (translational) velocity of the agent/robot
        :param omega: the requested rotational (angular) velocity of the agent/robot
        :return timeData: time data from telemetry
        :return poseData: Robot's/agent's pose
        :return sensorData: sensor data from telemetry
        :return imageData: Constructed omnicam image from the single images
        """
    ...
    def step_simulation_without_physics(self, newX, newY, newYaw):
        ...
        :param newX:       the global x position to teleport to
        :param newY:       the global y position to teleport to
        :param newYaw:     the global yaw value to teleport to
        :return timeData: time data from telemetry
        :return poseData: Robot's/agent's pose
        :return sensorData: sensor data from telemetry
        :return imageData: Constructed omnicam image from the single images
    ...
    def actuateRobot(self, actuatorCommand):
        ...
        :param actuatorCommand:  the command that is used in the actuation process
        :return timeData: time data from telemetry
        :return poseData: Robot's/agent's pose
        :return sensorData: sensor data from telemetry
        :return imageData: Constructed omnicam image from the single images
    ...
    def setXYYaw(self, objectName, pose):
        ...
        :param pose: the pose to teleport to
        :return: None
        """
    ...
    def setIllumination(self, lightSource, color):
        ...
        :param lightSource: the name of the light source to change
        :param color: the color of the light source (as [red,green,blue])
        :return: None
        """
    ...
    def getLimits(self):
        ...
        :param: None
        :return minX, maxX, minY, maxY: the minimum and maximum limits of the environment parameters in x,y coordinates
        """
    ...
    def getWallGraph(self):
        ...
        :param: None
        :return safeZoneVertices: environmental perimeter by means of wall vertices
        :return safeZoneSegments: environmental perimeter by means of wall segments
        """
...
class ImageInterface:
    """
    A very basic class for performing ABA renewal experiments from static image input. It is assumed that the agent sees
    in every node of a topology graph a static 360deg image of the environment. Herein, rotation of the agent is not
    enabled. This setup accelerates standard ABA renewal experiments, since the Blender rendering 'overhead' is not
    required. It is necessary that Blender is run prior to the application of this class, since the interface assumes
    a worldStruct data block in the 'world' directory of the experiment folder. This data is generated by a initial run
    of the getWorldInformation.py script in the experiment folder.
    """

    def __init__(self, imageSet='worldInfo/images.npy'):
        """
        Inits Image Interface class

        :param imageSet: the image set used for different contexts A & B
        :return: None
        """
        ...
        # flag for agent/robot at the goal node (this need NOT be the global goal node!)
        self.goalReached = False
    ...
    def getManuallyDefinedTopologyNodes(self):
        ...
        :param: None
        :return manuallyDefinedTopologyNodes: returns manually defined topology nodes from the environment
        """
    ...
    def getManuallyDefinedTopologyEdges(self):
        """
        This function reads all manually defined topology edges from the environment (if such nodes are defined)

        :param: None
        :return manuallyDefinedTopologyNodes: returns manually defined topology edges from the environment
        """
    ...
    def step_simulation_without_physics(self, newX, newY, newYaw):
        """
        This function propels the simulation. It uses teleportation to guide the agent/robot directly by
        means of global x,y,yaw values.

        :param newX: the global x position to teleport to
        :param newY: the global y position to teleport to
        :param newYaw: the global yaw value to teleport to
        :return timeData: data acquired from the robot
        :return poseData: data acquired from the robot
        :return sensorData: data acquired from the robot
        :return imageData: data acquired from the robot
        """
        # update robot's/agent's pose
    ...
    def actuateRobot(self, actuatorCommand):
        ...
        :param actuatorCommand: command used in the actuation process
        :return timeData: data acquired from the robot
        :return poseData: data acquired from the robot
        :return sensorData: data acquired from the robot
        :return imageData: data acquired from the robot
        ...
        if actuatorCommand.shape[0] > 2:
            ...
            # this is a standard motion command with physics support (the agent/robot approaches the goal by
            # actuating the robot's/agent's wheels).
            [timeData, poseData, sensorData, imageData] = self.stepSimulation(actuatorCommand[0], actuatorCommand[1])
    ...
    def setXYYaw(self, pose):
        ...
        :param pose: the goal pose for the agent/robot to teleport to
        :return: None
        """
        # send the request for teleportation to Blender
        poseStr = '%f,%f,%f' % (pose[0], pose[1], pose[2])
    ...
    def getLimits(self):
        """
        This function gets and return the limits of the environmental perimeter.
    ...
    def getWallGraph(self):
        """
        This function gets and returns environmental perimeter by means of wall vertices/segments.

        :param: None
        :return safeZoneVertices: set of safe vertices
        :return safeZoneSegments: set of safe segments
        """
        return self.safeZoneVertices, self.safeZoneSegments
