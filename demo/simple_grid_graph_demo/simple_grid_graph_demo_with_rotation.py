...
# shall the system provide visual output while performing the experiments? NOTE: do NOT use visualOutput=True in
# parallel experiments, visualOutput=True should only be used in explicit calls to 'singleRun'!
visualOutput = True
...
def rewardCallback(values):
    ...
    :returns reward: reward value
    :returns stopEpisode: flag set to true/false
    """
...
def singleRun():
    ...
    modules['spatial_representation'] = ManualTopologyGraphWithRotation(modules,
                                                                        {'startNodes': [0, 4, 8, 12], 'goalNodes': [15],
                                                                         'cliqueSize': 4})
