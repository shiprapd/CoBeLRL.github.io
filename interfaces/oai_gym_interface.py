...
class OAIGymInterface(gym.Env):
    """
    This is the Open AI gym interface class. The interface wraps the control path and ensures communication between
    the agent and the environment. The class descends from gym.Env, and is designed to be minimalistic
    """

    def __init__(self, modules, withGUI=True, rewardCallback=None):
        """
        Inits Open AI gym interface class, this interface wraps the control path and ensures communication between
        the agent and the environment.

        :param modules: the dict of all available modules in the system
        :param withGUI: if True, the module provides GUI control
        :param rewardCallback: this callback function is invoked in the step routine in order to get the appropriate reward w.r.t. the experimental design
        :return: None
        """
        # store the modules
    ...
    def updateObservation(self, observation):
        ...
        :param observation:  the observation used to perform the update
        :return: None
        """
    ...
    def _step(self, action):
        ...
        The action is used to decide on the next topology node to run to, and step then triggers the control path
        (including 'Blender')by making use of direct calls and signal/slot methods.

        :param action:   the action to be executed
        :return observation: observation after executing action
        :return reward: Reward for the action executed
        :return stopEpisode: flag to topEpisode
    ...
    def _reset(self):
        ...
        :param: None
        :return observation: observation after resetting RL agent's
        """
...
def unity_decorater(func):
    """
    Wraps on internal errors raised by the unity python api results in more readable error messages.

    :param: None
    :returns wrapper: Error message
    """
...
def get_cobel_path():
    """
    This function returns the cobel project path
    TODO:  move this to some kind of utility class?

    :param: None
    :return path: returns the cobel project path
    """
...
def get_env_path():
    """
    This function finds and returns the unity environment path.
    TODO move this to some kind of utility class?
...
class UnityInterface(gym.Env):
    """
    Wrapper for Unity 3D with ML-agents class
    """
    ...
    class UnityProcessor(Processor):
        """
        This is a class keras processor for the unity interface
        """
        ...
        def __init__(self, env_agent_specs,
            ...
            """
            Inits keras processor for the unity interface
            ...
            :param use_gray_scale: set gray scale use to true/false
            :param performance_monitor: for performance monitor
            :return: None
            """
        ...
        def __get_action_space(self, env_agent_specs, agent_action_type):
            ...
            :param env_agent_specs:     the group_spec object for the agent transmitted by ml agents env
            :param agent_action_type:   the agent_action_type string
            :return action_shape, action_space, action_type: tuple (action_shape, action_space, action_type) used by CoBeL-RL
            """
        ...
        def process_observation(self, observation):
            ...
            :param observation: An observation object as obtained by the environment
            :return observation: Observation obtained by the environment processed
            """
        ...
        def process_action(self, action):
            ...
            :param  action: Action given to the environment, data type: int
            :return action: Processed action given to the environment
            """
        ...
        def process_reward(self, reward):
            ...
            :param reward: A reward as obtained by the environment, data type: float
            :return reward:  Reward obtained by the environment processed
            """
            return reward

        def __process_observations(self, observations):
            """
            Format the received observation to work with cobel-rl.

            :param observations: the sensor observations received from ml-agents
            :return processed_observation: the formatted observations
            """
            # this means we have multiple sensors attached
        ...
        def __to_gray_scale_image(self, image_array):
            ...
            :param image_array : a 3D image array
            :return gray_scale_image: returns 2D grayscale image array
            """
    ...
    def __init__(self, env_path, scene_name=None,
        ...
        """
        Inits a constructor to connects to a given environment executable or to the unity editor and acts as a gym for
        keras agents.

        :param env_path:                full path to compiled unity executable, if None mlagents waits for the editor
                                        to connect
        :param modules:                 the old CoBeL-RL modules, (currently redundant)
        :param seed:                    Random seed, keep at 42 for good luck
        :param timeout_wait:            Time until Unity is declared dead
    ...
    def __get_step_results(self):
        ...
        :param: None
        :return observation, reward, done:  a tuple of observation, reward, done
        """
    ...
    def __start_editor_process(self, resource_path, scene_path, scene_name):
        ...
        :param resource_path: executable resource path
        :param scene_path: executable scene path
        :param scene_name: executable scene name
        :return : None
        """
